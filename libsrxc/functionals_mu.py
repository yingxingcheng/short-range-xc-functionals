...
import lrccd.libsrxc.functionals as func


def PW92c_mu(rho_c, rho_s, mu, parameters):
    ...
    n_arg, rs_arg = symbols('n_arg rs_arg', real=True)
    zeta = rho_s / rho_c
    rs = (3 / (4 * pi * rho_c)) ** (1 / 3)
    epsc = func.PW92c(rho_c, rho_s, parameters)
    ...
    gpp = (
            2 ** (5 / 3) / (5 * alpha ** 2 * rs_arg ** 2)
            * (1 + app * rs_arg) / (1 + bpp * rs_arg + cpp * rs_arg ** 2)
    )
    ...
    epsc_LR = (
            phi2 ** 3 * Q + a1 * mu ** 3 + a2 * mu ** 4 + a3 * mu ** 5 + a4 * mu ** 6 + a5 * mu ** 8
    )
...
def VWN5c_mu(rho_c, rho_s, mu, parameters):
    # rho_c = symbols('rho_c', real=True, nonnegative=True)
    # rho_s = symbols('rho_s', real=True)
    # 10.1103/PhysRevB.73.155111
    # n_arg, rs_arg, mu = symbols('n_arg rs_arg mu', real=True)
    ...
    zeta = rho_s / rho_c
    rs = (3 / (4 * pi * rho_c)) ** (1 / 3)
    epsc = func.VWN5c(rho_c, rho_s, parameters)
...
def PBEc_mu(rho_c, rho_s, gamma_cc, mu, parameters):
    # rho_c = symbols('rho_c', real=True, nonnegative=True)
    # rho_s = symbols('rho_s', real=True)
    ...
    beta = (betaPBE * (PW92c_mu(rho_c, rho_s, mu, parameters)
                       / func.PW92c(rho_c, rho_s, parameters)) ** alpha_c)
    A = beta / (gamma * (exp(-PW92c_mu(rho_c, rho_s, mu, parameters) / (gamma * phi ** 3)) - 1))
    H = gamma * phi ** 3 * ln(
        1 + beta * t ** 2 / gamma * ((1 + A * t ** 2) / (1 + A * t ** 2 + A ** 2 * t ** 4)))

    eps = PW92c_mu(rho_c, rho_s, mu, parameters) + H

    return eps


def TPSSc_mu_case_1(rho_c, rho_s, gamma_cc, gamma_cs, gamma_ss, tau_c, tau_s, mu, parameters):
    # rho_c = symbols('rho_c', real=True, nonnegative=True)
    # rho_s = symbols('rho_s', real=True)
    ...
    eps = PBEc_mu(rho_c, rho_s, gamma_cc, mu, parameters) + (
            func.TPSSc_case_1(rho_c, rho_s, gamma_cc, gamma_cs, gamma_ss, tau_c, tau_s, parameters)
            - func.PBEc(rho_c, rho_s, gamma_cc, parameters)
    ) * exp(-etac * mubar)

    return eps


def TPSSc_mu_case_2(rho_c, rho_s, gamma_cc, gamma_cs, gamma_ss, tau_c, tau_s, mu, parameters):
    # rho_c = symbols('rho_c', real=True, nonnegative=True)
    # rho_s = symbols('rho_s', real=True)
    # gamma_cc, gamma_ss = symbols('gamma_cc gamma_ss', real=True, nonnegative=True)
    # gamma_cs = symbols('gamma_cs', real=True)
    ...
    eps = PBEc_mu(rho_c, rho_s, gamma_cc, mu, parameters) + (
            func.TPSSc_case_2(rho_c, rho_s, gamma_cc, gamma_cs, gamma_ss, tau_c, tau_s, parameters)
            - func.PBEc(rho_c, rho_s, gamma_cc, parameters)
    ) * exp(-etac * mubar)
...
def TPSSc_mu_case_3(rho_c, rho_s, gamma_cc, gamma_cs, gamma_ss, tau_c, tau_s, mu, parameters):
    # rho_c = symbols('rho_c', real=True, nonnegative=True)
    # rho_s = symbols('rho_s', real=True)
    # gamma_cc, gamma_ss = symbols('gamma_cc gamma_ss', real=True, nonnegative=True)
    # gamma_cs = symbols('gamma_cs', real=True)
    ...
    eps = PBEc_mu(rho_c, rho_s, gamma_cc, mu, parameters) + (
            func.TPSSc_case_3(rho_c, rho_s, gamma_cc, gamma_cs, gamma_ss, tau_c, tau_s, parameters)
            - func.PBEc(rho_c, rho_s, gamma_cc, parameters)
    ) * exp(-etac * mubar)
...
def TPSSc_mu_case_4(rho_c, rho_s, gamma_cc, gamma_cs, gamma_ss, tau_c, tau_s, mu, parameters):
    # rho_c = symbols('rho_c', real=True, nonnegative=True)
    # rho_s = symbols('rho_s', real=True)
    # gamma_cc, gamma_ss = symbols('gamma_cc gamma_ss', real=True, nonnegative=True)
    # gamma_cs = symbols('gamma_cs', real=True)
    ...
    eps = PBEc_mu(rho_c, rho_s, gamma_cc, mu, parameters) + (
            func.TPSSc_case_4(rho_c, rho_s, gamma_cc, gamma_cs, gamma_ss, tau_c, tau_s, parameters)
            - func.PBEc(rho_c, rho_s, gamma_cc, parameters)
    ) * exp(-etac * mubar)
...
def LDAx_mu_case_1(rho, mu, parameters):
...
def LDAx_mu_case_2(rho, mu, parameters):
    # using LDAx from Dalton
    # rho_a = symbols('rho_a', real=True, nonnegative=True)
    # mu = symbols('mu', real=True, nonnegative=True)
...
def LDAx_mu_case_3(rho, mu, parameters):
    # using LDAx from Dalton
    # rho_a = symbols('rho_a', real=True, nonnegative=True)
    # mu = symbols('mu', real=True, nonnegative=True)
...
def PBEx_mu_case_1(rho, gamma, mu, parameters):
    # rho_a = symbols('rho_a', real=True, nonnegative=True)
    ...
    eps = LDAx_mu_case_1(rho, mu, parameters) * Fx

    return eps


def PBEx_mu_case_2_1(rho, gamma, mu, parameters):
    # rho_a = symbols('rho_a', real=True, nonnegative=True)
    # gamma_aa = symbols('gamma_aa', real=True, nonnegative=True)
    # mu = symbols('mu', real=True, nonnegative=True)
    ...
    berf = ((-7 + 72 * a ** 2) / (27 * (-3 - 24 * a ** 2 + 32 * a ** 4 + 8 * sqrt(pi) * a))) * fak
    Fx = 1 + kappa - kappa / (1 + berf * s ** 2 / kappa)
    eps = LDAx_mu_case_2(rho, mu, parameters) * Fx

    return eps


def PBEx_mu_case_2_2(rho, gamma, mu, parameters):
    # rho_a = symbols('rho_a', real=True, nonnegative=True)
    # gamma_aa = symbols('gamma_aa', real=True, nonnegative=True)
    # mu = symbols('mu', real=True, nonnegative=True)
...
def PBEx_mu_case_2_3(rho, gamma, mu, parameters):
    # rho_a = symbols('rho_a', real=True, nonnegative=True)
    # gamma_aa = symbols('gamma_aa', real=True, nonnegative=True)
    # mu = symbols('mu', real=True, nonnegative=True)
    ...
    fak2 = exp(0.25 / a ** 2)
    berf = (1.851851851851851851851852 * 10 ** -2 * (
            -1 + 1.44 * 10 ** 2 * a ** 4 * (-1 + fak2) - 2 * a ** 2 * (
            1.1 * 10 + 7 * fak2))) / (a ** 2 * (3.2 * 10 * a ** 4 * (
            -1 + fak2) - 3 * fak2 + 1.417963080724412821838534 * 10 * a * erf(
        5 * 10 ** -1 / a) * fak2 - 8 * a ** 2 * (-2 + 3 * fak2)))
    berf *= fak
    Fx = 1 + kappa - kappa / (1 + berf * s ** 2 / kappa)
    eps = LDAx_mu_case_2(rho, mu, parameters) * Fx
...
def PBEx_mu_case_3(rho, gamma, mu, parameters):
    # rho_a = symbols('rho_a', real=True, nonnegative=True)
    # gamma_aa = symbols('gamma_aa', real=True, nonnegative=True)
    # mu = symbols('mu', real=True, nonnegative=True)
    ...
    berf = (1 / (72 * a ** 2) - 1 / (17280 * a ** 4) - 23 / (358400 * a ** 6)) * fak
    Fx = 1 + kappa - kappa / (1 + berf * s ** 2 / kappa)
    eps = LDAx_mu_case_3(rho, mu, parameters) * Fx

    return eps


def TPSSx_mu_case_1(rho, gamma, tau, lapl, mu, parameters):
    # rho_a = symbols('rho_a', real=True, nonnegative=True)
    # gamma_aa = symbols('gamma_aa', real=True, nonnegative=True)
    ...
    eps = (
            PBEx_mu_case_1(rho, gamma, mu, parameters)
            + (func.TPSSx(rho, gamma, tau, lapl, parameters)
               - func.PBEx(rho, gamma, parameters)) * exp(-etax * mubar)
    )

    return eps


def TPSSx_mu_case_2_1(rho, gamma, tau, lapl, mu, parameters):
    # rho_a = symbols('rho_a', real=True, nonnegative=True)
    # gamma_aa = symbols('gamma_aa', real=True, nonnegative=True)
    # tau_a = symbols('tau_a', real=True)
    # mu = symbols('mu', real=True, nonnegative=True)
    ...
    eps = (
            PBEx_mu_case_2_1(rho, gamma, tau, parameters)
            + (func.TPSSx(rho, gamma, tau, lapl, parameters)
               - func.PBEx(rho, gamma, parameters)) * exp(-etax * mubar)
    )
...
def TPSSx_mu_case_2_2(rho, gamma, tau, lapl, mu, parameters):
    # rho_a = symbols('rho_a', real=True, nonnegative=True)
    # gamma_aa = symbols('gamma_aa', real=True, nonnegative=True)
    # tau_a = symbols('tau_a', real=True)
    # mu = symbols('mu', real=True, nonnegative=True)
    ...
    eps = (
            PBEx_mu_case_2_2(rho, gamma, mu, parameters)
            + (func.TPSSx(rho, gamma, tau, lapl, parameters)
               - func.PBEx(rho, gamma, parameters)) * exp(-etax * mubar)
    )
...
def TPSSx_mu_case_2_3(rho, gamma, tau, lapl, mu, parameters):
    # rho_a = symbols('rho_a', real=True, nonnegative=True)
    # gamma_aa = symbols('gamma_aa', real=True, nonnegative=True)
    # tau_a = symbols('tau_a', real=True)
    # mu = symbols('mu', real=True, nonnegative=True)
    ...
    eps = (
            PBEx_mu_case_2_3(rho, gamma, mu, parameters)
            + (func.TPSSx(rho, gamma, tau, lapl, parameters)
               - func.PBEx(rho, gamma, parameters)) * exp(-etax * mubar)
    )
...
def TPSSx_mu_case_3(rho, gamma, tau, lapl, mu, parameters):
    # rho_a = symbols('rho_a', real=True, nonnegative=True)
    # gamma_aa = symbols('gamma_aa', real=True, nonnegative=True)
    # tau_a = symbols('tau_a', real=True)
    # mu = symbols('mu', real=True, nonnegative=True)
    ...
    eps = (
            PBEx_mu_case_3(rho, gamma, mu, parameters)
            + (func.TPSSx(rho, gamma, tau, lapl, parameters)
               - func.PBEx(rho, gamma, parameters)) * exp(-etax * mubar))
...
def wPBEx(rho, gamma, mu, parameters):
    # rho_a = symbols('rho_a', real=True, nonnegative=True)
    # gamma_aa = symbols('gamma_aa', real=True, nonnegative=True)
    # mu = symbols('mu', real=True, nonnegative=True)
    ...
    H /= (
            1 + b1 * s + b2 * s ** 2 + b3 * s ** 3 + b4 * s ** 4 + b5 * s ** 5
            + b6 * s ** 6 + b7 * s ** 7 + b8 * s ** 8 + b9 * s ** 9
    )
