...
def PW92c(rho_c, rho_s, parameters):
...
def VWN5c(rho_c, rho_s, parameters):
    # rho_c = symbols('rho_c', real=True, nonnegative=True)
    # rho_s = symbols('rho_s', real=True)
...
def PBEc(rho_c, rho_s, gamma_cc, parameters):
    # rho_c = symbols('rho_c', real=True, nonnegative=True)
    # rho_s = symbols('rho_s', real=True)
    ...
    t2 = t**2
    ecunif = PW92c(rho_c, rho_s, parameters)
...
def TPSSc_case_1(rho_c, rho_s, gamma_cc, gamma_cs, gamma_ss, tau_c, tau_s, parameters):
    ...
    # NOTE from YingXing
    # gamma_cc = gamma_aa + 2*gamma_ab + gamma_bb
    # gamma_ss = gamma_aa - 2*gamma_ab + gamma_bb
    # gamma_cs = gamma_aa - gamma_bb
    ...
    epsbarc_alpha = spec_func.PBEc_alpha_replaced(rho_c, rho_s, gamma_cc, gamma_cs, gamma_ss, parameters)
    epsbarc_beta = spec_func.PBEc_beta_replaced(rho_c, rho_s, gamma_cc, gamma_cs, gamma_ss, parameters)
    epsrevPKZB = PBEc(rho_c, rho_s, gamma_cc, parameters) * ( 1 + C * (tauW/tau_c)**2 ) - (1 + C )*(tauW/tau_c)**2*(rho_a*epsbarc_alpha + rho_b*epsbarc_beta)/rho_c
...
def TPSSc_case_2(rho_c, rho_s, gamma_cc, gamma_cs, gamma_ss, tau_c, tau_s, parameters):
    ...
    C = Czero / ( 1 + eta**2 * ( (1+zeta)**(-4/3) + (1-zeta)**(-4/3) )/2 )**4
    epsbarc_alpha = spec_func.PBEc_alpha_replaced(rho_c, rho_s, gamma_cc, gamma_cs, gamma_ss, parameters)
    epsbarc_beta = PBEc(rho_c, rho_s, gamma_cc, parameters)
    epsrevPKZB = PBEc(rho_c, rho_s, gamma_cc, parameters) * ( 1 + C * (tauW/tau_c)**2 ) - (1 + C )*(tauW/tau_c)**2*(rho_a*epsbarc_alpha + rho_b*epsbarc_beta)/rho_c
...
def TPSSc_case_3(rho_c, rho_s, gamma_cc, gamma_cs, gamma_ss, tau_c, tau_s, parameters):
    ...
    eta = abs_grad_zeta / (2*(3*pi**2*rho_c)**(1/3))
    C = Czero / ( 1 + eta**2 * ( (1+zeta)**(-4/3) + (1-zeta)**(-4/3) )/2 )**4
    epsbarc_alpha = PBEc(rho_c, rho_s, gamma_cc, parameters)
    epsbarc_beta = spec_func.PBEc_beta_replaced(rho_c, rho_s, gamma_cc, gamma_cs, gamma_ss, parameters)
    epsrevPKZB = PBEc(rho_c, rho_s, gamma_cc, parameters) * ( 1 + C * (tauW/tau_c)**2 ) - (1 + C )*(tauW/tau_c)**2*(rho_a*epsbarc_alpha + rho_b*epsbarc_beta)/rho_c
...
def TPSSc_case_4(rho_c, rho_s, gamma_cc, gamma_cs, gamma_ss, tau_c, tau_s, parameters):
...
def PBEx(rho, gamma, parameters):
    ...
    eps = LDAx(rho, parameters) * Fx
    
    return eps
    
    
def TPSSx(rho, gamma, tau, lapl, parameters):
