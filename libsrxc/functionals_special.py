...
def PW92c_alpha_replaced(rho_c, rho_s, parameters):
...
def PW92c_beta_replaced(rho_c, rho_s, parameters):
    # Special version of PW92, used for PKZB functional
...
def PBEc_alpha_replaced(rho_c, rho_s, gamma_cc, gamma_cs, gamma_ss, parameters):
    ...
    t2 = t**2
    ecunif = PW92c_alpha_replaced(rho_c, rho_s, parameters)
...
def PBEc_beta_replaced(rho_c, rho_s, gamma_cc, gamma_cs, gamma_ss, parameters):
    # Special version of PBE, used for PKZB functional
    ...
    t2 = t**2
    ecunif = PW92c_beta_replaced(rho_c, rho_s, parameters)
    A = beta/gamma*(exp(-ecunif/(gamma*phi**3*e**2/a0))-1)**(-1)
    H = (e**2/a0)*gamma*phi**3*ln(1 + beta/gamma*t**2*(( 1 + A * t**2 )/( 1 + A * t**2 + A**2 * t**4 )))
    epsPBE = ecunif + H
